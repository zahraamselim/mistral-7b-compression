...
The key innovation is the use of approximate second-order information through the Hessian matrix:

\begin{equation}
H = \frac{2}{n}XX^T
\end{equation}

GPTQ employs a greedy coordinate descent approach, quantizing weights column-by-column while adjusting subsequent weights to compensate for quantization error. For each weight $w_q$ being quantized:
...
To balance precision and compression, GPTQ applies quantization in groups. Each group of $g$ weights (e.g., $g=128$) shares quantization parameters (scale $s$ and zero-point $z$):
...
\textbf{Activation Ordering:}

The optional \texttt{desc\_act} parameter reorders columns by activation magnitude to group similar-scale weights, potentially reducing quantization error:

\begin{equation}
\pi = \text{argsort}(\text{diag}(H)), \quad W \leftarrow W[:, \pi]
\end{equation}

\textbf{Quantization Process:}

We quantized Mistral-7B-v0.1 at three precision levels (4-bit, 3-bit, and 2-bit) using auto-gptq on a P100 GPU, with the following configuration:

\begin{itemize}
\item Base model: mistralai/Mistral-7B-v0.1 (7.24B parameters)
\item Bits: 2, 3, 4
\item Group size: 128 (default), 64 (quality experiment)
\item Calibration dataset: WikiText-2 (128-512 samples, max length 512)
\item Damping factor: 0.01 (regularization for Hessian stability)
\item desc\_act: False (basic), True (quality experiment)
\item Memory optimization: cache\_examples\_on\_gpu=False
\item Environment: PYTORCH\_CUDA\_ALLOC\_CONF="expandable\_segments:True"
\end{itemize}
...
model.quantize(
    ...
    use_triton=False,
    batch_size=1,
    cache_examples_on_gpu=False  # Critical for P100
)
...
The quantization process takes approximately 22 minutes per model on a P100 GPU, with memory usage optimized through CPU offloading and disabling GPU caching of calibration data.

\textbf{Optimized Inference:}

For evaluation, we employed ExLlamaV2, an optimized inference engine for GPTQ models that provides 2-3x faster inference through specialized CUDA kernels and efficient memory management:
...
ExLlamaV2 optimizations include:
\begin{itemize}
\item Custom CUDA kernels for 2/3/4-bit matrix multiplication
\item Fused attention and MLP operations
\item Lazy KV cache loading to reduce memory overhead
\item Efficient memory layout for quantized weights
\end{itemize}

\textbf{Quality Enhancement Experiments:}

To investigate quality-efficiency trade-offs, we conducted additional experiments beyond baseline configurations:

\begin{enumerate}
\item \textbf{Activation ordering} (desc\_act=True): Reorders weight columns by activation magnitude, grouping similar-scale weights to reduce intra-group variance:
\begin{lstlisting}[language=Python]
quantize_config=BaseQuantizeConfig(
    bits=3,
    group_size=128,
...
\item \textbf{Smaller group size} (group\_size=64): Reduces quantization granularity, allowing finer-grained scale/zero-point adaptation at the cost of 2x metadata storage:
\begin{lstlisting}[language=Python]
quantize_config=BaseQuantizeConfig(
    bits=3,
    group_size=64  # Halve group size
)
\end{lstlisting}

\item \textbf{Increased calibration} (512 samples): For 2-bit quantization, we quadrupled calibration data to provide more representative Hessian approximation:
\begin{lstlisting}[language=Python]
calibration_data = prepare_calibration_data(
    tokenizer, 
    n_samples=512  # 4x increase
)
\end{lstlisting}
\end{enumerate}

These configurations were systematically evaluated to characterize the quality-compression frontier across 2-4 bit precision levels, with results presented in Section~\ref{sec:results}.
