...
% \begin{itemize}
%     \item \textbf{SparseGPT:} One-shot unstructured pruning method that formulates pruning as a sparse regression problem, achieving high sparsity while maintaining model accuracy through layer-wise reconstruction. We target 50\% sparsity across all weight matrices using the Hessian-based approach.
    
%     \textbf{Parameters:}
%     \begin{itemize}
%         \item Sparsity level: 50\% (uniform across layers)
%         \item Pruning type: Unstructured (individual weight removal)
%         \item Calibration dataset: C4 (128 samples)
%         \item Reconstruction: Layer-wise with Hessian approximation
%     \end{itemize}
    
%     \textbf{Implementation:}
% \begin{lstlisting}[language=Python, caption={SparseGPT Unstructured Pruning}]
...
% data = load_dataset("allenai/c4", "en", split="train")
% calibration_samples = [data[i]['text'] for i in range(128)]

% for name, module in model.named_modules():
%     if isinstance(module, torch.nn.Linear):
%         # Initialize SparseGPT for this layer
%         pruner = SparseGPT(module)
...
%         pruner.prune(sparsity=0.5, prunen=0, prunem=0)
        
%         # Apply mask to weights
%         module.weight.data *= pruner.mask
% \end{lstlisting}

% \vspace{0.5em}

%     \item \textbf{Structured Pruning (Layer + Head Removal):} A two-stage structured pruning approach applied to the 4-bit quantized model, removing entire transformer layers and attention heads to achieve hardware-agnostic speedups without requiring sparse computation support.
    
%     \textbf{Stage 1 - Layer Pruning Parameters:}
%     \begin{itemize}
%         \item Original layers: 32
%         \item Target layers: 20 (37.5\% reduction)
%         \item Early layers kept: 8 (first 40\%)
%         \item Late layers kept: 12 (last 60\%)
%         \item Strategy: Preserve syntactic (early) and semantic (late) layers
%     \end{itemize}
    
%     \textbf{Stage 2 - Attention Head Pruning Parameters:}
%     \begin{itemize}
%         \item Reduction: 25\% of key-value heads per layer
%         \item Minimum heads: 1 per layer
%         \item Architecture: Exploits Grouped Query Attention (GQA)
%     \end{itemize}
    
%     \textbf{Implementation:}
% \begin{lstlisting}[language=Python, caption={Structured Pruning: Layer and Head Removal}]
% import torch.nn as nn

% # Stage 1: Layer Pruning - Keep 20 out of 32 layers
% total_layers = len(model.model.layers)
% keep_first = int(20 * 0.4)  # Keep 8 early layers (syntax)
% keep_last = 20 - keep_first  # Keep 12 late layers (semantics)

% # Select layers to preserve
% indices = (list(range(keep_first)) + 
%            list(range(total_layers - keep_last, total_layers)))

% # Create new layer list
% new_layers = nn.ModuleList([model.model.layers[i] for i in indices])
% model.model.layers = new_layers

% # Update model configuration
% model.config.num_hidden_layers = len(new_layers)

% print(f"Pruned from {total_layers} to {len(new_layers)} layers")

% # Stage 2: Attention Head Pruning - Reduce KV heads by 25%
% for layer_idx, layer in enumerate(model.model.layers):
%     attn = layer.self_attn
    
%     if hasattr(attn, 'num_key_value_heads'):
%         original_heads = attn.num_key_value_heads
%         new_heads = max(1, int(original_heads * 0.75))
        
%         if new_heads < original_heads:
%             # Reduce head count
%             attn.num_key_value_heads = new_heads
            
%             # Adjust head dimension
%             head_dim = attn.head_dim
%             new_kv_size = new_heads * head_dim
            
%             # Prune key and value projections
%             attn.k_proj.weight = nn.Parameter(
%                 attn.k_proj.weight[:new_kv_size, :]
%             )
%             attn.v_proj.weight = nn.Parameter(
%                 attn.v_proj.weight[:new_kv_size, :]
%             )
            
%             print(f"Layer {layer_idx}: {original_heads} -> "
%                   f"{new_heads} KV heads")
...
% \vspace{1em}
% \noindent Both pruning methods target different compression objectives: SparseGPT achieves fine-grained unstructured sparsity (50\%) suitable for specialized hardware, while structured pruning removes 37.5\% of layers and 25\% of attention heads for general-purpose acceleration.
...
% \textbf{Teacher-Student Configuration:}
% \begin{itemize}
%     \item \textbf{Teacher Model:} Qwen3-Next-80B-A3B-Instruct (80B parameters)
%     \item \textbf{Student Model:} Mistral-7B-v0.1 (7B parameters)
%     \item \textbf{Distillation Method:} Soft label learning with KL divergence loss
%     \item \textbf{Training Paradigm:} LoRA-based supervised fine-tuning
% \end{itemize}
...
%     \item Target modules: q\_proj, v\_proj (query and value projections)
%     \item Dropout: 0.1
%     \item Trainable parameters: ~100M (1.4\% of total)
%     \item Task type: Causal language modeling
% \end{itemize}

% \textbf{Training Configuration:}
% \begin{itemize}
%     \item Dataset: WikiText-2 train split (500 samples)
%     \item Learning rate: 2e-4
%     \item Batch size: 4 with gradient accumulation
%     \item Epochs: 3
%     \item Optimizer: AdamW (8-bit)
%     \item Temperature: 2.0 (for softening teacher outputs)
%     \item Loss: $\alpha \cdot \mathcal{L}_{\text{KL}} + (1-\alpha) \cdot \mathcal{L}_{\text{CE}}$ where $\alpha = 0.5$
% \end{itemize}

% \textbf{Implementation:}
% \begin{lstlisting}[language=Python, caption={Knowledge Distillation with LoRA}]
% from transformers import AutoModelForCausalLM, AutoTokenizer
% from peft import LoraConfig, get_peft_model, TaskType
% import torch
% import torch.nn.functional as F

% # Load teacher model (80B)
...
% # Output: trainable params: 100M || all params: 7.1B || 
% #         trainable%: 1.4%

% # Load training data
% from datasets import load_dataset
% data = load_dataset("wikitext", "wikitext-2-raw-v1", split="train")
% train_samples = data.select(range(500))
...
%     output_dir="./mistral-7b-distilled",
...
\begin{table}[H]
\centering
\caption{Efficiency comparison of quantization methods on Tesla T4 GPU}
\label{tab:quant_efficiency}
\small
\begin{tabular}{lccccccccc}
\hline
\textbf{Method} & \textbf{Size} & \textbf{Memory} & \textbf{Latency} & \textbf{TTFT} & \textbf{Throughput} & \textbf{MFU} & \textbf{Energy} & \textbf{Prefill} & \textbf{Decode} \\
 & \textbf{(GB)} & \textbf{(MB)} & \textbf{(ms/tok)} & \textbf{(ms)} & \textbf{(tok/s)} & \textbf{(\%)} & \textbf{(mJ/tok)} & \textbf{(ms)} & \textbf{(ms/tok)} \\
\hline
FP16 & 13.49 & 7013.03 & 62.97 & 68.84 & 15.84 & 2.36 & 3085.58 & 69.85 & 61.36 \\
NF4 & 3.74 & 1859.11 & 81.12 & 177.16 & 12.30 & 1.83 & 3974.65 & 187.97 & 79.09 \\
GPTQ & 3.87 & 4415.90 & 1136.80 & 1113.00 & 0.88 & 0.13 & 55703.12 & 1110.89 & 1117.76 \\
AWQ & 3.87 & 1869.04 & 78.68 & 79.25 & 12.79 & 1.91 & 3855.44 & 92.76 & 77.22 \\
HQQ & 3.74 & 4721.1 & 661.65 & 664.73 & 1.51 & 0.23 & 32420.79 & 663.17 & 649.84 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Memory and compression details for quantization methods}
\label{tab:quant_memory}
\small
\begin{tabular}{lcccccc}
\hline
\textbf{Method} & \textbf{Total} & \textbf{Bits per} & \textbf{Memory} & \textbf{KV Cache} & \textbf{Compression} & \textbf{Memory} \\
 & \textbf{Params} & \textbf{Param} & \textbf{Efficiency} & \textbf{(MB)} & \textbf{Ratio} & \textbf{Reduction} \\
\hline
FP16 & 7.24B & 16.0 & 1.97 & 1024.0 & 1.00x & 1.00x \\
NF4 & 7.24B & 4.0 & 2.06 & 1024.0 & 3.61x & 3.77x \\
GPTQ & 7.24B & 4.0 & 0.90 & 1024.0 & 3.48x & 1.59x \\
AWQ & 7.24B & 4.0 & 2.12 & 1024.0 & 3.49x & 3.75x \\
HQQ & 7.24B & 4.0 & 0.81 & 1024.0 & 3.61x & 1.49x \\
\hline
\end{tabular}
\end{table}
...
% \begin{table}[H]
% \centering
% \caption{Efficiency comparison of pruning methods on Tesla T4 GPU}
% \label{tab:pruning_efficiency}
% \small
% \begin{tabular}{lccccccccc}
% \hline
% \textbf{Method} & \textbf{Size} & \textbf{Memory} & \textbf{Latency} & \textbf{TTFT} & \textbf{Throughput} & \textbf{MFU} & \textbf{Energy} & \textbf{Prefill} & \textbf{Decode} \\
...
% \begin{table}[H]
% \centering
% \caption{Memory and compression details for pruning methods}
% \label{tab:pruning_memory}
% \small
% \begin{tabular}{lcccccc}
% \hline
% \textbf{Method} & \textbf{Total} & \textbf{Sparsity/} & \textbf{Memory} & \textbf{KV Cache} & \textbf{Compression} & \textbf{Speedup} \\
%  & \textbf{Params} & \textbf{Reduction} & \textbf{Efficiency} & \textbf{(MB)} & \textbf{Ratio} & \textbf{vs Baseline} \\
% \hline
...
% \begin{table}[H]
% \centering
% \caption{Efficiency comparison of knowledge distillation on Tesla T4 GPU}
% \label{tab:distill_efficiency}
% \small
% \begin{tabular}{lccccccccc}
% \hline
% \textbf{Method} & \textbf{Size} & \textbf{Memory} & \textbf{Latency} & \textbf{TTFT} & \textbf{Throughput} & \textbf{MFU} & \textbf{Energy} & \textbf{Prefill} & \textbf{Decode} \\
...
% \begin{table}[H]
% \centering
% \caption{Memory and training details for knowledge distillation}
% \label{tab:distill_memory}
% \small
% \begin{tabular}{lcccccc}
% \hline
% \textbf{Method} & \textbf{Total} & \textbf{Trainable} & \textbf{Memory} & \textbf{Training} & \textbf{Compression} & \textbf{Speedup} \\
%  & \textbf{Params} & \textbf{Params} & \textbf{Efficiency} & \textbf{Time} & \textbf{Ratio} & \textbf{vs Baseline} \\
% \hline
...
% \begin{table}[H]
% \centering
% \caption{Performance comparison of pruning methods across benchmarks}
% \label{tab:pruning_performance}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{Perplexity} & \textbf{HellaSwag} & \textbf{ARC-Easy} & \textbf{ARC-Challenge} & \textbf{GSM8K} & \textbf{MMLU} & \textbf{HumanEval} \\
...
% \begin{table}[H]
% \centering
% \caption{Average accuracy and performance degradation for pruning methods}
% \label{tab:pruning_performance_summary}
% \small
% \begin{tabular}{lcccc}
% \hline
% \textbf{Method} & \textbf{Average} & \textbf{Perplexity} & \textbf{Accuracy} & \textbf{Tasks} \\
...
% \begin{table}[H]
% \centering
% \caption{Performance comparison of knowledge distillation across benchmarks}
% \label{tab:distill_performance}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{Perplexity} & \textbf{HellaSwag} & \textbf{ARC-Easy} & \textbf{ARC-Challenge} & \textbf{GSM8K} & \textbf{MMLU} & \textbf{HumanEval} \\
...
% \begin{table}[H]
% \centering
% \caption{Average accuracy and performance degradation for knowledge distillation}
% \label{tab:distill_performance_summary}
% \small
% \begin{tabular}{lcccc}
% \hline
% \textbf{Method} & \textbf{Average} & \textbf{Perplexity} & \textbf{Accuracy} & \textbf{Tasks} \\
...
% \begin{table}[H]
% \centering
% \caption{IR-style retrieval metrics for quantization methods}
% \label{tab:quant_ir_metrics}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{P@1} & \textbf{P@3} & \textbf{P@5} & \textbf{R@1} & \textbf{R@3} & \textbf{MRR} & \textbf{MAP} \\
% \hline
% FP16 & --- & --- & --- & --- & --- & --- & --- \\
% NF4 & --- & --- & --- & --- & --- & --- & --- \\
% GPTQ & --- & --- & --- & --- & --- & --- & --- \\
...
% \begin{table}[H]
% \centering
% \caption{RAG answer quality evaluation of pruning methods}
% \label{tab:pruning_rag}
% \small
% \begin{tabular}{lcccccccc}
% \hline
% \textbf{Method} & \textbf{F1} & \textbf{EM} & \textbf{Faithful-} & \textbf{Relevance} & \textbf{ROUGE-1} & \textbf{ROUGE-2} & \textbf{ROUGE-L} & \textbf{BERTScore} \\
...
% \begin{table}[H]
% \centering
% \caption{RAG vs no-RAG comparison for pruning methods}
% \label{tab:pruning_rag_comparison}
% \small
% \begin{tabular}{lcccccc}
% \hline
% \textbf{Method} & \textbf{No-RAG} & \textbf{No-RAG} & \textbf{F1} & \textbf{EM} & \textbf{Avg Answer} & \textbf{Avg Answer} \\
...
% \begin{table}[H]
% \centering
% \caption{Context retrieval quality for pruning methods}
% \label{tab:pruning_retrieval}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{Sufficiency} & \textbf{Precision} & \textbf{Coverage} & \textbf{Consistency} & \textbf{Avg Score} & \textbf{Avg Chunks} & \textbf{Avg Context} \\
...
% \begin{table}[H]
% \centering
% \caption{IR-style retrieval metrics for pruning methods}
% \label{tab:pruning_ir_metrics}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{P@1} & \textbf{P@3} & \textbf{P@5} & \textbf{R@1} & \textbf{R@3} & \textbf{MRR} & \textbf{MAP} \\
...
% \begin{table}[H]
% \centering
% \caption{RAG efficiency metrics for pruning methods}
% \label{tab:pruning_rag_efficiency}
% \small
% \begin{tabular}{lcccccc}
% \hline
% \textbf{Method} & \textbf{Retrieval} & \textbf{RAG Gen} & \textbf{No-RAG Gen} & \textbf{RAG} & \textbf{No-RAG} & \textbf{Generation} \\
...
% \begin{table}[H]
% \centering
% \caption{RAG answer quality evaluation of knowledge distillation}
% \label{tab:distill_rag}
% \small
% \begin{tabular}{lcccccccc}
% \hline
% \textbf{Method} & \textbf{F1} & \textbf{EM} & \textbf{Faithful-} & \textbf{Relevance} & \textbf{ROUGE-1} & \textbf{ROUGE-2} & \textbf{ROUGE-L} & \textbf{BERTScore} \\
...
% \begin{table}[H]
% \centering
% \caption{RAG vs no-RAG comparison for knowledge distillation}
% \label{tab:distill_rag_comparison}
% \small
% \begin{tabular}{lcccccc}
% \hline
% \textbf{Method} & \textbf{No-RAG} & \textbf{No-RAG} & \textbf{F1} & \textbf{EM} & \textbf{Avg Answer} & \textbf{Avg Answer} \\
...
% \begin{table}[H]
% \centering
% \caption{Context retrieval quality for knowledge distillation}
% \label{tab:distill_retrieval}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{Sufficiency} & \textbf{Precision} & \textbf{Coverage} & \textbf{Consistency} & \textbf{Avg Score} & \textbf{Avg Chunks} & \textbf{Avg Context} \\
...
% \begin{table}[H]
% \centering
% \caption{IR-style retrieval metrics for knowledge distillation}
% \label{tab:distill_ir_metrics}
% \small
% \begin{tabular}{lccccccc}
% \hline
% \textbf{Method} & \textbf{P@1} & \textbf{P@3} & \textbf{P@5} & \textbf{R@1} & \textbf{R@3} & \textbf{MRR} & \textbf{MAP} \\
...
% \begin{table}[H]
% \centering
% \caption{RAG efficiency metrics for knowledge distillation}
% \label{tab:pruning_rag_efficiency}
% \small
% \begin{tabular}{lcccccc}
% \hline
